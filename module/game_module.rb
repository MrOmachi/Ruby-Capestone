...
include GamesAuthor
  ...
  def add_game
    ...
    game_data = {last_played_date: last_played_date, multiplayer: multiplayer.downcase == 'y' ? true : false, authors: author_arr }
  ...
  def list_game
    ...
    game.each_with_index { |x, i| puts "\n #{i + i}. Game with Last Play Date: #{x['last_played_date']} and multiplayer: #{x['multiplayer'] == 'enabled' ? 'true' : 'False' } \n \n"}
  end
